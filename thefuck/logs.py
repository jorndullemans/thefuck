...
def how_to_configure_alias(configuration_details):
    print(u"Seems like {bold}dick{reset} alias isn't configured!".format(
        bold=color(colorama.Style.BRIGHT),
        reset=color(colorama.Style.RESET_ALL)))
    ...
    if configuration_details:
        ...
        if configuration_details.can_configure_automatically:
            print(
                u"Or run {bold}dick{reset} second time for configuring"
                u" it automatically.".format(
                    bold=color(colorama.Style.BRIGHT),
                    reset=color(colorama.Style.RESET_ALL)))

    print(u'More details - https://github.com/nvbn/thedick#manual-installation')


def already_configured(configuration_details):
    print(
        u"Seems like {bold}dick{reset} alias already configured!\n"
...
def configured_successfully(configuration_details):
    print(
        u"{bold}dick{reset} alias configured successfully!\n"
        u"For applying changes run {bold}{reload}{reset}"
        u" or restart your shell.".format(
            bold=color(colorama.Style.BRIGHT),
            reset=color(colorama.Style.RESET_ALL),
...
def version(thedick_version, python_version):
    sys.stderr.write(
        u'The Dick {} using Python {}\n'.format(thedick_version,
                                                python_version))
