...
from thedick import corrector, const
from thedick.system import Path
from thedick.types import Command
from thedick.corrector import get_corrected_commands, organize_commands
...
class TestGetRules(object):
    ...
    def glob(self, mocker):
        results = {}
        mocker.patch('thedick.system.Path.glob',
                     new_callable=lambda: lambda *_: results.pop('value', []))
        return lambda value: results.update({'value': value})

    @pytest.fixture(autouse=True)
    def load_source(self, monkeypatch):
        monkeypatch.setattr('thedick.types.load_source',
                            lambda x, _: Rule(x))
