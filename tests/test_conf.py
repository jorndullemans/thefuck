...
from thedick import const


@pytest.fixture
def load_source(mocker):
    return mocker.patch('thedick.conf.load_source')
...
class TestSettingsFromEnv(object):
    def test_from_env(self, os_environ, settings):
        os_environ.update({'THEDICK_RULES': 'bash:lisp',
                           'THEDICK_EXCLUDE_RULES': 'git:vim',
                           'THEDICK_WAIT_COMMAND': '55',
                           'THEDICK_REQUIRE_CONFIRMATION': 'true',
                           'THEDICK_NO_COLORS': 'false',
                           'THEDICK_PRIORITY': 'bash=10:lisp=wrong:vim=15',
                           'THEDICK_WAIT_SLOW_COMMAND': '999',
                           'THEDICK_SLOW_COMMANDS': 'lein:react-native:./gradlew'})
        settings.init()
    ...
    def test_from_env_with_DEFAULT(self, os_environ, settings):
        os_environ.update({'THEDICK_RULES': 'DEFAULT_RULES:bash:lisp'})
        settings.init()
...
@pytest.mark.parametrize('legacy_dir_exists, xdg_config_home, result', [
    (False, '~/.config', '~/.config/thedick'),
    (False, '/user/test/config/', '/user/test/config/thedick'),
    (True, '~/.config', '~/.thedick'),
    (True, '/user/test/config/', '~/.thedick')])
def test_get_user_dir_path(mocker, os_environ, settings, legacy_dir_exists,
                           xdg_config_home, result):
    mocker.patch('thedick.conf.Path.is_dir',
