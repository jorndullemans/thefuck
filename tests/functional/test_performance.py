...
WORKDIR /src
USER test
RUN echo 'eval $(thedick --alias)' > /home/test/.bashrc
...
def plot(proc, TIMEOUT):
    ...
    proc.sendline(u'dick')
    assert proc.expect([TIMEOUT, u'No dicks given'])
...
def test_performance(spawnu, TIMEOUT, benchmark):
    proc = spawnu(u'thedick/python3-bash-performance',
                  dockerfile, u'bash')
